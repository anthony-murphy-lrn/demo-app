...
const envSchema = z.object({
  ...
  LEARNOSITY_CONSUMER_KEY: z.string().min(1, 'Learnosity consumer key is required'),
  LEARNOSITY_CONSUMER_SECRET: z.string().min(1, 'Learnosity consumer secret is required'),
  LEARNOSITY_DOMAIN: z.string().min(1, 'Learnosity domain is required'),
  LEARNOSITY_ACTIVITY_ID: z.string().min(1, 'Learnosity activity ID is required').default('quickstart_examples_activity_template_001'),
